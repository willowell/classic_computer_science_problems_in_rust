...
impl Edge for UnweightedEdge {
    ...
    fn reversed(&self) -> Self {
        UnweightedEdge { uv: EdgeUV { u: self.uv.v, v: self.uv.u } }
    }
}
...
struct Graph<V, E> where E: Edge + PartialEq {
    vertices: Vec<V>,
    edges: Vec<E>,
}

impl<V, E> Graph<V, E> where V: Clone + PartialEq, E: Clone + Edge + PartialEq {
    fn new(vertices: Vec<V>) -> Self {
        Self { vertices, edges: Default::default() }
    }
...
impl<V, E> fmt::Display for Graph<V, E> where V: Clone + fmt::Display + fmt::Debug + PartialEq, E: Clone + Edge + PartialEq {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self
            .vertices
            .iter()
            .map(|v| writeln!(f, "{} -> {:?}", v, self.get_neighbours_for(v))).collect()
    }
}
...
impl<V> UnweightedGraph<V> {
    fn new(vertices: Vec<V>) -> Self {
        Self {
            graph: Graph { vertices, edges: vec![] }
        }
    }
}

impl<V> fmt::Display for UnweightedGraph<V> where V: Clone + fmt::Display + fmt::Debug + PartialEq {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.graph)
    }
}
...
fn main() {
    let mut ug = UnweightedGraph::new(vec![
        "Atlanta",
        "Boston",
        "Chicago"
    ]);
