...
impl WordGrid {
    ...
    fn new_with_setup(rows: usize, columns: usize) -> Self {
    ...
    fn mark_word(&mut self, word: &str, locations: Vec<GridLocation>) {
        ...
        while i < word.len() {
            ...
            if let Some(location) = location {
                self.grid[location.row as usize][location.column as usize] =
        ...
        for row in 0..self.rows {
            for column in 0..self.columns {
                if column + word_length <= self.columns {
                    if let Some(right_locations) =
                        Self::get_right_locations(row, column, word_length)
                    {
                        domain.push(right_locations);
                    }

                    if row + word_length <= self.rows {
                        if let Some(diagonal_right_locations) =
                            Self::get_diagonal_right_locations(row, column, word_length)
                        {
                ...
                if row + word_length <= self.rows {
                    ...
                    if (column as i32 - word_length as i32) >= 0 {
                        if let Some(diagonal_left_locations) =
                            Self::get_diagonal_left_locations(row, column, word_length)
                        {
    ...
    fn get_diagonal_right_locations(
        row: usize,
        column: usize,
        word_length: usize,
    ) -> Option<Vec<GridLocation>> {
    ...
    fn get_diagonal_left_locations(
        row: usize,
        column: usize,
        word_length: usize,
    ) -> Option<Vec<GridLocation>> {
...
impl fmt::Display for WordGrid {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        for row in &self.grid {
            for col in row {
                write!(f, "{}{}{}", style("[").dim(), col, style("]").dim())?;
            }
            writeln!(f)?;
        }
